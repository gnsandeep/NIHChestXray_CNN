...
def checkpoint(model, best_loss, epoch, LR):
    ...
    torch.save(state, 'DenseNet/checkpoint')
...
def get_cnn_model():
    ...
    """
        Defines the CNN model here. Used densenet121 pretrained model
        replaced the last classifier layer (model.classifier) with a nn.Linear layer and sigmoid
...
def train_model(model, train_dataloader, val_dataloader, n_epoch=n_epochs, optimizer=optimizer, criterion=criterion):
    ...
    for epoch in range(n_epoch):
        ...
        for column in true_df_epoc:
            ...
            pred_df_epoc.to_csv("DenseNet/" + str(epoch) + "_DenseNetpredsSGDMomentum.csv", index=False)
            auc_df.to_csv("DenseNet/" + str(epoch) + "_DenseNetaucsSGDMomentum.csv", index=False)

        pred_df_epoc = pd.DataFrame(columns=["Image Index"])
        true_df_epoc = pd.DataFrame(columns=["Image Index"])
    
    train_epochs_loss.to_csv("DenseNet/train_epochs_loss.csv", index=False)
    val_epochs_loss.to_csv("DenseNet/val_epochs_loss.csv", index=False)
    checkpoint_best = torch.load('DenseNet/checkpoint')
...
pred_df.to_csv("DenseNet/DenseNetpredsSGDMomentum.csv", index=False)
auc_df.to_csv("DenseNet/DenseNetaucsSGDMomentum.csv", index=False)
